...
def render(component_function, component_id):
    ...
    for index, effect_data in effects_to_run.items():
        ...
        if old_deps is None:
            ...
            should_run = True
            print(f"  -> Efeito no índice {index} será executado (dependências mudaram).")
        else:
            print(f"  -> Efeito no índice {index} não será executado (dependências não mudaram).")
            
        if should_run:
            if index in component_cleanups:
                print(f"    -> Função de limpeza do efeito no índice {index} será executada.")
...
def MeuComponenteComEfeitos():
    ...
    useEffect(
        lambda: print("      >> EFEITO 1: Componente montado! Isto só aparece uma vez."),
    ...
    def efeito_do_contador():
        ...
        def cleanup():
            print(f"      << CLEANUP 2: Limpando o efeito do contador (quando o valor era {count}).")
        return cleanup
